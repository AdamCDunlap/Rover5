...
int main(int argc, char* argv[]) {
    ...
    while (ardport) {
        ...
        if (checkForValue(ardport, '0')) continue;
        ardport.read((char *)&stmicros, 4);
        if (checkForValue(ardport, '1')) continue;
        ardport.read((char *)&encdsts[Rover5::FL], 4);
        if (checkForValue(ardport, '2')) continue;
        ardport.read((char *)&encdsts[Rover5::FR], 4);
        if (checkForValue(ardport, '3')) continue;
        ardport.read((char *)&encdsts[Rover5::BL], 4);
        if (checkForValue(ardport, '4')) continue;
        ardport.read((char *)&encdsts[Rover5::BR], 4);

        if (checkForValue(ardport, '5')) continue;
        ardport.read((char *)&pows[Rover5::FL], 2);
        if (checkForValue(ardport, '6')) continue;
        ardport.read((char *)&pows[Rover5::FR], 2);
        if (checkForValue(ardport, '7')) continue;
        ardport.read((char *)&pows[Rover5::BL], 2);
        if (checkForValue(ardport, '8')) continue;
        ardport.read((char *)&pows[Rover5::BR], 2);
        if (checkForValue(ardport, '9')) continue;

        ardport.read((char *)&xpos, 4);
        if (checkForValue(ardport, 'A')) continue;
        ardport.read((char *)&ypos, 4);
        if (checkForValue(ardport, 'B')) continue;
        ardport.read((char *)&ang, 4);
        if (checkForValue(ardport, 'C')) continue;

        ardport.read((char *)&endmicros, 4);
        if (checkForValue(ardport, 'D')) continue;
